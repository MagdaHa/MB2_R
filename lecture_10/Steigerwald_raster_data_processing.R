################################################
#### Steigerwald 10: Raster data processing ####
################################################

####skript which generates the dataset for the Steigerwald tasks####

#---------https://github.com/wegmann/R_scripts/blob/master/Steigerwald_data_preparation.R------------#
...
guf <- raster("GUF_EAGLE_AOI_UTM_WGS84.tif")  #  #guf=global urban footprint
srtm <- raster("SRTM.tif")
s2 <- brick("S2_2017_06_19.tif")

lucas.2 <- projectRaster(lucas,s2, method = "ngb")   #ngb= nearest neighbour, categorical
